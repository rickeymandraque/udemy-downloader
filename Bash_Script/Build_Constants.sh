...
out_dir=$1
Conf_Dir=".conf/Udemy-DL"
keyfile="$Conf_Dir/keyfile.json"
cookies="$Conf_Dir/cookies.txt"
logs_path="$Conf_Dir/logs"



constants_file="$(cat << EOF
import logging
import os
import time
...
SAVED_DIR = os.path.join(os.getcwd(), "${saved_dir}")
KEY_FILE_PATH = os.path.join(os.getcwd(), "${keyfile}")
COOKIE_FILE_PATH = os.path.join(os.getcwd(), "${cookies}")
LOG_DIR_PATH = os.path.join(os.getcwd(), "${logs_path}")
LOG_FILE_PATH = os.path.join(
    os.getcwd(), "${logs_path}", f"{time.strftime('%Y-%m-%d-%I-%M-%S')}.log")
